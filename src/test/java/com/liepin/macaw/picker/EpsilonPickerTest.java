...
public class EpsilonPickerTest {
    ...
    public void testPicker(){
        ...
        try {
            EpsilonGreedyPicker picker = DataPickers.newEpsilonGreedyPicker(policies, params, epsilon);
            PolicyCounter counter = new PolicyCounter();
            for (int i = 0; i < 1000; i++) {
                counter.addPolicy(picker.pick(String.valueOf(i)));
            }
            counter.print();
            assert counter.getPolicyCount("P1") > 700;
            assert counter.getPolicyCount("P2") > 50;
            assert counter.getPolicyCount("P3") > 50;
            assert counter.getPolicyCount("P4") > 50;
